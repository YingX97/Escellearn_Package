...
class h5Reader:
    ...
    def read_file(self, df_type,genes=None):
        ...
        if genes is None:
            ...
            genes = genes.split(",")
        with h5py.File(self.filename,"r") as h5_data:
            # List of genes
            indexs = []
            if isinstance(genes,list):
                for gene in genes:
                    indexs.append(self.L.index(gene))
            else:
                indexs.append(self.L.index(genes))
            # sort the indexs
            indexs.sort()
            new_genes = []
            for index in indexs:
    ...
    def __detect_outlier(self, expression_list):
        threshold=4
        ...
        return dict(zip(outliers, z_scores))
    
    # given a gene name
    # find out which celltype has an abnormally high/low expression value
    def __get_outlier_celltype(self, df, gene_name):
        cell_type_candidate = {}
        target_df = df.loc[[gene_name]]
        outlier_values = self.__detect_outlier([val for val in target_df.values[0] if val > 0])
        for celltype in target_df.columns:
            if target_df[celltype].values[0] in outlier_values:
                cell_type_candidate[celltype] = outlier_values[target_df[celltype].values[0]]
        return cell_type_candidate
    
    def load_marker_genes(self):
        """
        Determine marker genes for each celltype based on expression value.
        Definition of marker gene: for a gene to be a marker gene for a celltype,
        the average expression of the gene in that celltype must be a positive outlier.

        :return: a dictionary with celltypes being keys and marker genes being values
        :rtype: dict
        """
        df = self.read_file("celltype")
        cell_marker_combines = {}
        for celltype in df.columns:
            possible_marker_genes = {}
            for gene in df.index:
                outlier_cells = self.__get_outlier_celltype(df, gene)
                if celltype in outlier_cells:
                    possible_marker_genes[gene] = outlier_cells[celltype]
            gene_sorted_by_z_score = [k for k, v in sorted(possible_marker_genes.items(), key=lambda item: item[1])]
            cell_marker_combines[celltype] = gene_sorted_by_z_score
        
        self.marker_genes = cell_marker_combines
        return cell_marker_combines
    ...
    def get_marker_genes_list(self,celltype=None):
        ...
        return self.marker_genes[celltype]
